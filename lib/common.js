...
const {forEach, keys, has, get, unset, isObject, isFunction, reduce} = require('lodash');
...
const foreignProps = (compare, base) => {
    ...
    return reduce(keysInCompare, (result, key) => {
        ...
        if (!present) result.push(key);
            ...
            if (!baseValIsObject && !compareValIsObject) return result;

            // If the key is present in the "base" object, but the value is an object
            // whereas the "compare" value is not, we can treat it as present
            // as there is most likely a corresponding form control at the object-level
            if (baseValIsObject && !compareValIsObject) return result;
...
const splitErrors = (errors, schema) => {
    ...
    const foreign = foreignProps(errors, schema);
    const form    = [];
...
module.exports = {foreignProps, splitErrors};